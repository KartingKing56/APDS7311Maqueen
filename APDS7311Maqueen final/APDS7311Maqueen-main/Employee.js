...
if (valid) {
    ...
    // Submit or proceed further logic (e.g., send data to server)
}
...
router.post('/login', loginLimiter, async (req, res) => {
    try {
        ...
        // Validate username (email or account number)
        if (emailPattern.test(username)) {
            // It's an email, proceed to look up in the database
            console.log("Email format validated.");
        } else if (!accountPattern.test(username)) {
            // If not a valid account number format
            return res.status(400).json({ message: 'Invalid username (email/account number) format.' });
        }

        // Handle employee login or customer login
        if (username.includes("emp")) {
            ...
            // Customer login logic (using email or account number)
