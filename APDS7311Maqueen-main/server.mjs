...
// SSL configuration for both portals (Customer and Employee)
const sslOptions = {
...
app.use((req, res, next) => {
    // Checks if the request is not secure and redirects to HTTPS
    if (req.secure) {
        return next(); // If the connection is secure, continue
    } else {
        res.redirect('https://' + req.headers.host + req.url); // Redirects to HTTPS
    }
});
...
const PORT = process.env.PORT || 3443; // Using port 3443 for secure connection
